...
- [How to install and start NiFi Registry](#how-to-install-and-start-nifi-registry)
- [Recommended Antivirus Exclusions](#recommended-antivirus-exclusions)
- [Security Configuration](#security_configuration)
- User Authentication
  - [Lightweight Directory Access Protocol (LDAP)](#ldap_identity_provider)
  - [Kerberos](#kerberos_identity_provider)
- Authorization
  - [Authorizer Configuration](#authorizer-configuration)
  - [Authorizers.xml Setup](#authorizers-setup)
  - [Access Policies](#access-policies)
- Encrypted Passwords in Configuration Files
  - [Encrypt-Config Tool](#encrypt-config_tool)
  - [Sensitive Property Key Migration](#sensitive-property-key-migration)
- [Bootstrap Properties](#bootstrap_properties)
- [Proxy Configuration](#proxy_configuration)
- Kerberos Service
  - [Notes](#kerberos_service_notes)
- System Properties
  - [Web Properties](#web-properties)
  - [Security Properties](#security-properties)
  - [Identity Mapping Properties](#identity-mapping-properties)
  - [Providers Properties](#providers-properties)
  - [Alias Properties](#alias-properties)
  - [Database Properties](#database-properties)
  - [Extension Directories](#extension-directories)
  - [Kerberos Properties](#kerberos_properties)
- Metadata Database
  - [H2](#h2)
  - [Postgres](#postgres)
  - [MySQL](#mysql)
- [Schema Differences & Limitations](#schema-differences-limitations)
- Persistence Providers
  - [Flow Persistence Providers](#flow-persistence-providers)
  - [Bundle Persistence Providers](#bundle-persistence-providers)
- Event Hooks
  - [Shared Event Hook Properties](#shared-event-hook-properties)
  - [ScriptEventHookProvider](#scripteventhookprovider)
  - [LoggingEventHookProvider](#loggingeventhookprovider)
- [URL Aliasing](#url-aliasing)
- Backup & Recovery
  - [Metadata Database](#metadata-database-2)
  - [Persistence Providers](#persistence-providers-2)
  - [Bundle Persistence](#bundle-persistence)
...
See the [System Properties](#system_properties) section of this guide for more information about NiFi Registry configuration files.
...
See also [Kerberos Service](#kerberos_service) to allow single sign-on access via client Kerberos tickets.
...
See [Kerberos Properties](#kerberos_properties) for complete documentation.
...
These properties pertain to various security features in NiFi Registry. Many of these properties are covered in more detail in the [Security Configuration](#security_configuration) section.
...
|      | These mappings are also applied to the "Initial Admin Identity" in the *authorizers.xml* file, as well as users imported from LDAP (See [Authorizers.xml Setup](#authorizers-setup)). |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
These properties pertain to flow persistence providers. NiFi Registry uses a pluggable flow persistence provider to store the content of the flows saved to the registry. For further details on persistence providers, refer [Persistence Providers](#persistence-providers).
...
These properties pertain to the support for URL aliasing. For further details, refer to [URL Aliasing](#url-aliasing).
...
Each persistence provider has its own configuration parameters, which can be configured in an XML file specified in *[nifi-registry.properties](#providers-properties)*.
...
| **Property**             | **Description**                                              |
| ------------------------ | ------------------------------------------------------------ |
| `Flow Storage Directory` | REQUIRED: File system path for a directory where flow contents files are persisted to. The directory must exist when NiFi registry starts. Also must be initialized as a Git directory. See [Initialize Git directory](#initialize-git-directory) for detail. |
...
|      | This provider must be added to the classpath by specifying a custom extension directory in *nifi-registry.properties*, such as `nifi.registry.extension.dir.aws=./ext/aws/lib`, where `./ext/aws/` contains the contents of the extracted *nifi-registry-aws-assembly-<version>-bin.zip*. |
| ---- | ------------------------------------------------------------ |
|      |                                                              |
...
| Property Name            | Description                                                  |
| :----------------------- | :----------------------------------------------------------- |
| `Whitelisted Event Type` | Event types the hook provider configured with this property should respond to. If this property is left blank or not provided, all events will fire for the configured hook provider. Multiple `Whitelisted Event Type` can be specified and often are. For example, `<property name="Whitelisted Event Type 1">CREATE_FLOW</property>` and `<property name="Whitelisted Event Type 2">UPDATE_FLOW</property>` would invoke the configured hook provider for the `CREATE_FLOW` and `UPDATE_FLOW` event types. |
...
The aliases are configured in an XML file which can be specified in *[nifi-registry.properties](#alias-properties)*.
...
Last updated 2020-04-02 15:26:36 +02:00